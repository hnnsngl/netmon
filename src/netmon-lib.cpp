...
HostListItem sortFullMessage(const std::string &host, const std::string &message)
{
	...
	if (std::regex_search(system, res, sysreg)) {
		...
		ss1 << res[1];
		ss1 >> tmpitem.avgload1;
		ss2 << res[2];
		ss2 >> tmpitem.avgload2;
		ss3 << res[3];
		ss3 >> tmpitem.avgload3;
		tmpitem.load = res[1];
	} else {
	...
	if (std::regex_search(cpuinfo, res, reg))
		tmpitem.cpuname = res[1];
	else
		tmpitem.cpuname = "No Info";
	...
	if (std::regex_search(cpuinfo, res, reg)) {
		ss << res[1];
		ss >> tmpitem.processors;
	} else
		tmpitem.processors = std::numeric_limits<int>::quiet_NaN();
...
void printHostList(bool printProcesses)
{
	...
	for (auto &host : hostList) {
		...
		std::cout << "Load: " << host.second.avgload1 << ", " << host.second.avgload2 << ", "
		          << host.second.avgload3 << "\n";
