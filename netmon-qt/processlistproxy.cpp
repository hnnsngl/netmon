...
NetmonProcessListProxy::NetmonProcessListProxy(HostSelections &selected, QObject *parent)
    : QSortFilterProxyModel(parent), hostSelected(selected), userFilter(false), userFilterColumn(-1),
      userFilterString(getenv("USER")), commandFilter(false), commandFilterString("")
{
}

bool NetmonProcessListProxy::lessThan(const QModelIndex &left, const QModelIndex &right) const
{
...
bool NetmonProcessListProxy::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
